---
description: List MiniMax-M3 jobs (or inspect a specific job by id).
allowed-tools: ["Bash"]
argument-hint: "[<job-id>] [--json]"
---

You are running the `/minimax-m3:status` command.
...
```bash
node "${CLAUDE_PLUGIN_ROOT}/scripts/minimax-m3-companion.mjs" status [<args>]
```
