---
description: List GLM jobs (or inspect a specific job by id).
allowed-tools: ["Bash"]
argument-hint: "[<job-id>] [--json]"
---

You are running the `/glm:status` command.
...
```bash
node "${CLAUDE_PLUGIN_ROOT}/scripts/glm-companion.mjs" status [<args>]
```
