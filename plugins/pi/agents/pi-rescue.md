...
name: pi-rescue
description: |
  Use this agent for *longer or more involved* pi delegations where the user wants the work tracked as a job — versus the simple `pi` agent which is for quick one-shot questions. Trigger phrases: "/pi:rescue", "pi rescue", "백그라운드로 pi한테 시켜", "오래 걸리는 작업 pi한테 위임", "complex refactor with pi".

  <example>
  Context: 사용자가 큰 리팩터링을 pi에 백그라운드로 위임
  user: "이 폴더 전체 리팩터링하는 거 pi한테 백그라운드로 시켜줘"
  assistant: "pi-rescue 에이전트를 호출해 백그라운드 잡으로 등록하겠습니다."
  <commentary>
  ...
  <example>
  Context: 사용자가 명시적으로 /pi:rescue 사용
  user: "/pi:rescue 새 인증 모듈 설계 초안 작성"
  assistant: "pi-rescue 에이전트를 통해 작업을 위임하겠습니다."
  <commentary>
  ...
  user: "아까 pi한테 시킨 거 어떻게 됐어?"
  assistant: "pi-rescue 에이전트로 /pi:status 후 /pi:result를 실행하겠습니다."
  <commentary>
