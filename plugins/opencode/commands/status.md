---
description: List opencode jobs (or inspect a specific job by id).
allowed-tools: ["Bash"]
argument-hint: "[<job-id>] [--json]"
---

You are running the `/opencode:status` command.
...
```bash
node "${CLAUDE_PLUGIN_ROOT}/scripts/opencode-companion.mjs" status [<args>]
```
