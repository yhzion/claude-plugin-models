...
const manifestPath = resolve(repoRoot, 'plugins/pi/.claude-plugin/plugin.json');

test('pi plugin manifest is valid JSON', () => {
  const raw = readFileSync(manifestPath, 'utf8');
  assert.doesNotThrow(() => JSON.parse(raw));
});

test('pi plugin manifest has required fields', () => {
  const manifest = JSON.parse(readFileSync(manifestPath, 'utf8'));
  assert.equal(manifest.name, 'pi', 'plugin name must be "pi"');
...
test('pi plugin manifest author uses yhzion personal account', () => {
  const manifest = JSON.parse(readFileSync(manifestPath, 'utf8'));
  assert.equal(manifest.author?.name, 'yhzion');
  assert.equal(manifest.author?.email, 'gplusit@gmail.com');
});

test('pi plugin manifest points to yhzion/claude-plugin-models repository', () => {
  const manifest = JSON.parse(readFileSync(manifestPath, 'utf8'));
  assert.ok(
