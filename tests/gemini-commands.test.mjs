...
const commandsDir = resolve(repoRoot, 'plugins/gemini/commands');
...
test('all gemini v0.1.0 slash commands are present', () => {
...
for (const name of EXPECTED) {
  test(`gemini commands/${name}.md has description and allowed-tools frontmatter`, () => {
  ...
  test(`gemini commands/${name}.md invokes gemini-companion.mjs via \${CLAUDE_PLUGIN_ROOT}`, () => {
    const raw = readFileSync(join(commandsDir, `${name}.md`), 'utf8');
    assert.ok(
      raw.includes('${CLAUDE_PLUGIN_ROOT}/scripts/gemini-companion.mjs'),
