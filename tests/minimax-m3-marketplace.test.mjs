...
test('marketplace lists the minimax-m3 plugin', () => {
  const mp = JSON.parse(readFileSync(marketplacePath, 'utf8'));
  assert.ok(Array.isArray(mp.plugins), 'plugins must be an array');
  const mm = mp.plugins.find((p) => p.name === 'minimax-m3');
  assert.ok(mm, 'minimax-m3 plugin entry must exist');
  assert.match(mm.version ?? '', /^\d+\.\d+\.\d+$/);
  assert.ok(
    mm.description?.toLowerCase().includes('minimax'),
    'minimax-m3 entry description must mention MiniMax'
  );
});

test('marketplace minimax-m3 source path resolves to the minimax-m3 plugin manifest', () => {
  const mp = JSON.parse(readFileSync(marketplacePath, 'utf8'));
  const mm = mp.plugins.find((p) => p.name === 'minimax-m3');
  const sourcePath = resolve(repoRoot, mm.source);
