...
const commandsDir = resolve(repoRoot, 'plugins/opencode/commands');
...
test('all opencode core slash commands are present', () => {
...
test('opencode has no review command (core set)', () => {
  const files = readdirSync(commandsDir).filter((f) => f.endsWith('.md'));
  assert.ok(!files.includes('review.md'), 'opencode core set must not ship a review command');
});

for (const name of EXPECTED) {
  test(`opencode commands/${name}.md has description and allowed-tools frontmatter`, () => {
  ...
  test(`opencode commands/${name}.md invokes opencode-companion.mjs via \${CLAUDE_PLUGIN_ROOT}`, () => {
    const raw = readFileSync(join(commandsDir, `${name}.md`), 'utf8');
    assert.ok(
      raw.includes('${CLAUDE_PLUGIN_ROOT}/scripts/opencode-companion.mjs'),
